...
- **Owner**: the broadest application-level permissions within their store, subject to deliberate domain restrictions — not literally "full read/write". Owner-exclusive: store identity (`stores.name`/`phone`/`address` — owner-only `UPDATE` policy on `stores`), employee/invite administration (`create_store_invite`), approval decisions (`decide_approval_request`), and cash-closing review (`review_daily_cash_closing`). Like every other role, owner has **no product `DELETE` path** and cannot write `products.current_stock` directly — those stay RPC-controlled (see below) regardless of role.
